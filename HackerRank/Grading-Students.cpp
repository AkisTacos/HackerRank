...
/* Description:
    ...
    according to these rules:
        ...
        the next multiple of 5 is less than 3, round grade up 
...
int roundUp(int num, int multiple){
    ...
    return (multiple == 0 || mod == 0 || (multiple-mod) > 2) ? num : (num + multiple - mod);
}

int calcScore(int score){
    return (score < 38) ? score : roundUp(score, 5);
}

int main(){
    // Given input: n (number of students) then each line after is grades
    int numStudent, i, score;
    cin >> numStudent;

    for(i = 0; i < numStudent; i++){
        cin >> score;
        cout << calcScore(score) << endl;
    }
